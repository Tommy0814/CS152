...
class BinOpExpr implements Expression {
    ...
    public Value evaluate(Environment env) {
        Value result;
        switch (op){
            case ADD: result = new IntVal((e1.evaluate(env).toInt() + e2.evaluate(env).toInt())); 
                                break;
            case  SUBTRACT: result = new IntVal((e1.evaluate(env).toInt() - e2.evaluate(env).toInt())); 
                                break;
            case  MULTIPLY: result = new IntVal((e1.evaluate(env).toInt() * e2.evaluate(env).toInt())); 
                                break;
            case  DIVIDE: result = new IntVal((e1.evaluate(env).toInt() / e2.evaluate(env).toInt())); 
                                break;
            case  MOD: result = new IntVal((e1.evaluate(env).toInt() % e2.evaluate(env).toInt())); 
                                break;
            case  GT: result = new BoolVal((e1.evaluate(env).toInt() > e2.evaluate(env).toInt())); 
                                break;
            case  GE: result = new IntVal((e1.evaluate(env).toInt() >= e2.evaluate(env).toInt())); 
                                break;
            case  LT: result = new IntVal((e1.evaluate(env).toInt() < e2.evaluate(env).toInt())); 
                                break;
            case  LE: result = new IntVal((e1.evaluate(env).toInt() <= e2.evaluate(env).toInt())); 
                                break;
            case  EQ: result = new IntVal((e1.evaluate(env).toInt() == e2.evaluate(env).toInt())); 
                                break;
            default: result = new NullVall(); 
                                break;
