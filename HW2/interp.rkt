...
(define (eval-if c thn els env)
  (let [(condresult (car (evaluate c env)))]
    (if condresult (evaluate thn env) (evaluate els env))
        ))
...
(define (eval-while c body env)
  (let [(result (evaluate c env))]
    (if (car result)
        (begin
          (let [(resultbody
          (evaluate body env))]
          (eval-while c body (cdr resultbody))))
        (cons 0 (cdr result))
     )
   )
 )
...
(define (eval-assign var exp env)
  (let [(result (evaluate exp env))]
    (let* ((head (car result)) (tail (if (pair? result) (cdr result) (cdr (evaluate result env)))))
    (cons head (hash-set tail var head)))))



;; Handles sequences of statements
(define (eval-seq e1 e2 env)
  (let [(newenv (cdr (evaluate e1 env)))]
    (evaluate e2 newenv)))